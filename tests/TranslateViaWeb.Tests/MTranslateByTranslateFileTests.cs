...
public class MTranslateByTranslateFileTests
{
    ...
    public void Translate_Success()
    {
        ...
        using (var translater = new MTranslateByTranslateFile("./Data/MTranslateByTranslateFile.txt", config))
        {
            var res = translater.Translate();
            res.Should().BeTrue();
        }

        var text = File.ReadAllText("./out/MTranslateByTranslateFile.en.ru.txt");
        text.Should().Be("оправдание");
    }
}